...
exports.weeklyReport = client_1.inngest.createFunction({ id: 'weekly-report', name: 'Weekly Performance Report' }, { cron: '0 9 * * 1' }, async ({ step }) => {
    ...
    const sentCount = await step.run('send-reports', async () => {
        ...
        for (const user of eligibleUsers) {
            try {
                ...
                const postedVideos = await prisma_1.prisma.video.findMany({
                    where: {
                        userId: user.id,
                        status: 'posted',
                        postedAt: { gte: sevenDaysAgo },
                ...
                const totalFollowers = connections.reduce((sum, c) => sum + c.followerCount, 0);
                // Send email
                ...
                console.error(`[weeklyReport] Failed for user ${user.id}:`, userError);
            }
        }
        return sent;
    });
