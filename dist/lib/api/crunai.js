...
exports.buildSoraPrompt = buildSoraPrompt;
 ...
 */
function buildSoraPrompt(params) {
...
async function generateVideo(params) {
    const duration = params.duration ?? constants_1.AI_VIDEO_DURATION.default;
    const response = await axios_1.default.post(`${BASE_URL}/CreateTask`, {
        model: constants_1.CRUN_AI_MODELS.sora2,
        input: {
...
async function getVideoStatus(taskId) {
    const response = await axios_1.default.get(`${BASE_URL}/TaskInfo`, {
 ...
 */
async function pollUntilComplete(taskId, timeoutMs = 300000, // 5 minutes
    ...
    while (Date.now() - startTime < timeoutMs) {
        ...
        if (task.status === 'success') {
            ...
            return {
                ...
                duration: task.duration_s ?? constants_1.AI_VIDEO_DURATION.default,
            };
        }
 ...
 */
async function downloadAndUploadVideo(params) {
    // Download video buffer from Crun AI
    const videoResponse = await axios_1.default.get(params.videoUrl, {
        responseType: 'arraybuffer',
        timeout: 120000, // 2 min timeout for large files
    });
    const videoBuffer = Buffer.from(videoResponse.data);
    const videoKey = (0, storage_1.generateVideoKey)(params.userId, params.videoId);
    const gcsVideoUrl = await (0, storage_1.uploadBuffer)(videoBuffer, videoKey, 'video/mp4');
    return { gcsVideoUrl };
}
