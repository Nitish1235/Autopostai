...
exports.hexToASSColor = hexToASSColor;
exports.buildASSStyle = buildASSStyle;
exports.framesToTime = framesToTime;
exports.buildWordEvents = buildWordEvents;
exports.buildASSFile = buildASSFile;
exports.writeASSFile = writeASSFile;
...
// ── Build ASS Style ──────────────────────────────────
function buildASSStyle(subtitleConfig) {
    ...
    const marginV = Math.round(40 + (subtitleConfig.position / 100) * (1920 - 80 - subtitleConfig.fontSize));
...
function buildPhraseText(words, activeIndex, config) {
    const parts = [];
...
// ── Build Word Events ────────────────────────────────
function buildWordEvents(params) {
    ...
    for (let i = 0; i < wordTimestamps.length; i++) {
        ...
        if (currentPhrase.length === maxWordsPerLine ||
...
function buildASSFile(params) {
    ...
    // Events section
    const eventLines = events.map((e) => `Dialogue: 0,${e.start},${e.end},Default,,0,0,0,,${e.text}`);
    const eventsSection = [
        '[Events]',
...
async function writeASSFile(content, outputPath) {
